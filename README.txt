...
Run all commands after entering the /midsem folder(not from the CS305-2019CSB1112-midsem folder).
...
To run the tests, you have to make certain changes
    ...
    (ii) The functions in the DatabaseHandlerTest.java can be run individually and tested. Simulatenous run of these functions sometimes result in an SQLITE_BUSY exception, which is due to inherent behaviour of SQLite in case of multiple connections.
    (iii) Also change the dbURL in DatabaseHandlerTest.java, with the URL of your database.
    (iv) For the success of the function destinationFinder(), make sure the corresponding entry is present in the routing_table of your database.
